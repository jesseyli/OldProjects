...
def draw_top_states(top_states):
    get_canvas().draw_text('Top 5 Most Talkative States', (10, 5), 'Black',
                           'Arial', 12, style='bold')
    if top_states is None:
        get_canvas().draw_text('None', (10, 24))
    else:
        for i in range(1,6):
            if len(top_states) == 0:
                break
            state = top_states.pop(0)
            get_canvas().draw_text(state[0] + ": " + str(state[1]) + ' tweets',
                                  (10, 10 + i*14))
