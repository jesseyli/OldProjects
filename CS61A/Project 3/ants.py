...
Name: Jesse Li, Meghna Grover
Login: cs61a-gm, cs61a-ei
TA: Albert Wu
Section: 104
"""
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        if self.armor - amount <= 0:
            list_copy = list(self.place.bees)
...
class PrincessAnt(Ant):
    ...
    def action(self, colony):
        ...
        if not self.taken_action:
            current_place = self.place
            while current_place.entrance != colony.hive:
                current_place = current_place.entrance
            while current_place != colony.queen:
                if current_place.ant and current_place.ant != self:
                    current_place.ant.armor += 1
