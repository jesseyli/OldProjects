...
def draw_number(n, dot='*'):
    ...
    | $   $ |
     -------
    """
    return draw_dice(n % 2, n > 1, n > 3, n == 6, dot)
...
def play(strategy0, strategy1):
    ...
    score = [0,0] #score[0] = player 0 score, score[1] = player 1 score
    while True:
        if who == 0:
            num_rolls = strategy0(score[who], score[other(who)])
        else:
            num_rolls = strategy1(score[who], score[other(who)])
        num_rolls = min(num_rolls, num_allowed_dice(score[who], score[other(who)]))
        score[who] += take_turn(num_rolls, score[other(who)], select_dice(score[who], score[other(who)]), name(who))
        if score[who] >= goal:
            return who
        who = other(who)
