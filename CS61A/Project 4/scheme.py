...
def apply_primitive(procedure, args, env):
    ...
    if procedure.use_env:
        arglist.append(env)
    try:
        return procedure.fn(*arglist)
    except TypeError:
        raise SchemeError('invalid procedure arguments')
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        parameter = target[0]
        check_formals(Pair(parameter, nil))
        env.define(parameter, do_lambda_form(Pair(target.second, vals.second), env)) 
        return parameter
    else:
...
def check_formals(formals):
    ...
    for formal in formals:
        if not scheme_symbolp(formal) or formal in checked:
            raise SchemeError('{0} is not a valid parameter list'.format(formals))
        checked.append(formal)
