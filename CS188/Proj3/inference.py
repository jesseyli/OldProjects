...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        0.0
        """
        rand_samp = random.random()*self.total()
        prev = 0
        for key in self:
            if self[key]:
                if rand_samp <= self[key] + prev:
                    return key
                prev += self[key]
...
class ExactInference(InferenceModule):
    ...
    def update(self, observation, gameState):
        ...
        for key in self.beliefs:
            if key in self.allPositions:
                self.beliefs[key] = self.beliefs[key]*self.getObservationProb(observation, gameState.getPacmanPosition(), key,self.getJailPosition())
        self.beliefs.normalize()
    ...
    def predict(self, gameState):
        ...
        for key in self.allPositions:
            distributions[key] = self.getPositionDistribution(gameState, key)
        for key in self.allPositions:
            total = 0
            for i in distributions:
                total += distributions[i][key]*self.beliefs[i]
            values[key] = total
        for key in self.beliefs:
            self.beliefs[key] = values[key]
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = []
        p_per_loc = self.numParticles // len(self.legalPositions)
        remainder = self.numParticles % len(self.legalPositions)
        for pos in self.legalPositions:
            for _ in range(0,p_per_loc):
    ...
    def update(self, observation, gameState):
        ...
        """
        dist = DiscreteDistribution()
        unique = set(self.particles)
        for particle in unique:
            dist[particle] = self.particles.count(particle)*self.getObservationProb(observation, gameState.getPacmanPosition(), 
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        p_per_loc = self.numParticles // num_perm
        remainder = self.numParticles % num_perm
        for _ in range(0,p_per_loc):
            random.shuffle(permutations)
            self.particles += permutations
        random.shuffle(permutations)
        self.particles += permutations[:remainder]
    ...
    def update(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                weight = weight*self.getObservationProb(observation[i], gameState.getPacmanPosition(), 
