...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in actions:
            if self.qvalues[(state,action)] > best:
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        maxActionQ = -float('inf')
        if not actions:
            maxActionQ = 0
        for a in actions:
            if self.getQValue(nextState, a) > maxActionQ:
                maxActionQ = self.getQValue(nextState, a)
        difference = reward + self.discount*maxActionQ - self.getQValue(state,action)
        for feature in features:
