...
def enhancedFeatureExtractorDigit(datum):
    ...
    #pdb.set_trace()
    data = copy.deepcopy(datum.getPixels())
    counter = 0
    def flip(a,b):
        if a >= 0 and b >= 0:
            if a < DIGIT_DATUM_WIDTH and b < DIGIT_DATUM_HEIGHT:
                if data[a][b] == 0:
                    data[a][b] = 3
                    flip(a+1,b)
                    flip(a-1,b)
    ...
    sections = [0,0,0]
    sections[counter-1] = 1
    sections = sections + sections + sections
    sections2 = np.zeros(4)
    sections2[total//100-4] = 1
    sections = np.append(sections,sections2)


    '''
    total = 0
    lists = datum.getPixels()
    ...
    #print sections
    #f = open('test.txt','a')
    #f.write(datum.getAsciiString())
    #f.write('\n@@@@@@@@@@@@@@@@@@@@@@@\n')
    return np.append(features,sections)
